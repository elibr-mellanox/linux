...
static struct vport *ip6gre_tnl_create(const struct vport_parms *parms)
{
	...
	rtnl_lock();
	dev = ip6gretap_fb_dev_create(net, parms->name, NET_NAME_USER);
	if (IS_ERR(dev)) {
		rtnl_unlock();
...
static int __init ovs_ip6gre_tnl_init(void)
{
	return ovs_vport_ops_register(&ovs_ip6gre_vport_ops);
}

static void __exit ovs_ip6gre_tnl_exit(void)
{
	ovs_vport_ops_unregister(&ovs_ip6gre_vport_ops);
}

module_init(ovs_ip6gre_tnl_init);
module_exit(ovs_ip6gre_tnl_exit);

MODULE_DESCRIPTION("OVS: IPv6 GRE switching port");
MODULE_LICENSE("GPL");
MODULE_ALIAS("vport-type-109");
